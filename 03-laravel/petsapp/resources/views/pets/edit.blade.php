...
<form method="post" action="{{ route('pets.store') }}" class="pt-6 pb-20" enctype="multipart/form-data">
        ...
        <fieldset class="fieldset w-md bg-base-200 border border-base-300 p-4 rounded-box">
                ...
                {{-- old('name', $pet->name) para que se vea la informacion actual de la mascota en cada campo del 
                formulario de editaar--}}
                <label class="fieldset-label">Pet Name:</label>
                <input type="text" name="name" class="input rounded-full w-full" placeholder="Max" value="{{ old('name', $pet->name) }}" />
                ...
                <input type="number" step="0.1" name="weight" class="input rounded-full w-full" placeholder="5.5" value="{{ old('weight', $pet->weight) }}"/>

                <label class="fieldset-label">Age (years):</label>
                <input type="number" name="age" class="input rounded-full w-full" placeholder="2" value="{{ old('age', $pet->age) }}"/>

                <label class="fieldset-label">Breed:</label>
                <input type="text" name="breed" class="input rounded-full w-full" placeholder="Golden Retriever" value="{{ old('breed', $pet->breed) }}"/>

                <label class="fieldset-label">Location:</label>
                <input type="text" name="location" class="input rounded-full w-full" placeholder="Bogotá, Colombia" value="{{ old('location', $pet->location) }}"/>
